...
def telegram_success():
    requests.post(url=f'https://api.telegram.org/bot{TG_TOKEN}/sendMessage', data={'chat_id': TG_CHAT_ID, 'text': '✅ Витрина sales_mart обновлена', 'parse_mode': 'HTML'})

@dag(
    dag_id='build_sales_mart',
    start_date=datetime.datetime(year=2026, month=4, day=1),
    schedule='0 5 * * *',  # в 5 утра (после обновления справочника в 4)
    catchup=False,
