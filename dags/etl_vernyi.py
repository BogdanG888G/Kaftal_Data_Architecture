...
def telegram_success():
    requests.post(url=f'https://api.telegram.org/bot{TG_TOKEN}/sendMessage', data={'chat_id': TG_CHAT_ID, 'text': '✅ Паплайн по Верный завершился', 'parse_mode': 'HTML'})

default_args = {'retries': 2, 'retry_delay': datetime.timedelta(minutes=1)}

@dag(dag_id='etl_vernyi', start_date=datetime.datetime(year=2026, month=2, day=1), schedule='@daily', catchup=False,
     description='Автоматизация обработки данных для Верный', tags=['Spark', 'S3', 'vernyi'],
...
def pipeline():
    ...
    spark_processing = SparkSubmitOperator(task_id='spark_processing', conn_id='spark_connection',
        trigger_rule=TriggerRule.ALL_DONE, application='jobs/vernyi_spark_processing.py',
