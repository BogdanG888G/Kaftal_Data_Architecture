...
def telegram_success(context):
    ...
    ds = context.get("ds", "unknown")
    message = (
        f"✅ <b>Пайплайн по Глобус успешно завершился</b>\n"
...
@dag(
    ...
    schedule="@daily",
    catchup=False,
    description="Автоматизация обработки данных для Глобус",
    tags=["Spark", "S3", "globus"],
...
def pipeline():
    ...
    wait_for_files = FileSensor(
        ...
        filepath="/opt/airflow/data/globus*.csv",
        mode="reschedule",
        poke_interval=30,
        timeout=60,
    ...
    def load_to_s3_raw():
        ...
        for file in os.listdir(input_folder):
            if file.startswith("globus") and file.endswith(".csv"):
        ...
        if files_processed == 0:
            logging.warning("⚠ Нет файлов Глобус")
        else:
    ...
    spark_processing = SparkSubmitOperator(
        ...
        application="jobs/globus_spark_processing.py",
        on_failure_callback=telegram_alert,
        retries=1,
