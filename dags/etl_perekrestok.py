...
def telegram_success(context):
    ...
    message = (
        f"✅ <b>Пайплайн по Перекресток успешно завершился</b>\n"
...
@dag(
    dag_id="etl_perekrestok",
    start_date=datetime.datetime(year=2026, month=2, day=1),
    schedule="@daily",
    catchup=False,
    description="Автоматизация обработки данных для Перекресток",
    tags=["Spark", "S3", "perekrestok"],
...
def pipeline():
    ...
    wait_for_files = FileSensor(
        ...
        filepath="/opt/airflow/data/perekrestok*",
        mode="reschedule",
        poke_interval=30,
        timeout=60,
        soft_fail=True,
    ...
    spark_processing = SparkSubmitOperator(
        ...
        application="jobs/perekrestok_spark_processing.py",
        on_failure_callback=telegram_alert,
        retries=1,
