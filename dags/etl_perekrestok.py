...
def telegram_success():
    requests.post(url=f'https://api.telegram.org/bot{TG_TOKEN}/sendMessage', data={'chat_id': TG_CHAT_ID, 'text': '✅ Паплайн по Перекресток завершился', 'parse_mode': 'HTML'})

default_args = {'retries': 2, 'retry_delay': datetime.timedelta(minutes=1)}

@dag(dag_id='etl_perekrestok', start_date=datetime.datetime(year=2026, month=2, day=1), schedule='@daily', catchup=False,
     description='Автоматизация обработки данных для Перекресток', tags=['Spark', 'S3', 'perekrestok'],
...
def pipeline():
    ...
    wait_for_files = FileSensor(task_id='wait_for_files', fs_conn_id='folder_connect',
        filepath='/opt/airflow/data/perekrestok*', mode='reschedule', poke_interval=30,
    ...
    def load_to_s3_raw():
        ...
        for file in os.listdir('/opt/airflow/data/'):
            if file.startswith('perekrestok'):
    ...
    spark_processing = SparkSubmitOperator(task_id='spark_processing', conn_id='spark_connection',
        trigger_rule=TriggerRule.ALL_DONE, application='jobs/perekrestok_spark_processing.py',
