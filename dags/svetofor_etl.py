...
def telegram_success(context):
    send_telegram('Пайплайн по Светофору успешно завершился ✅')


@dag(
    dag_id='svetofor_etl',
    description='Автоматизация обработки данных Светофора',
    catchup=False,
    start_date=datetime.datetime(year=2026, month=4, day=1),
    schedule='@daily',
    tags=['Svetofor', 'S3', 'Spark', 'Konditerka'],
...
def pipeline():
    ...
    spark_processing = SparkSubmitOperator(
        ...
        application='jobs/svetofor_spark_processing.py',
        on_failure_callback=telegram_alert,
        deploy_mode='client',
        conf={
            'spark.jars.packages': (
