...
def telegram_success():
    message = '✅ Паплайн по Ашан успешно завершился'
    requests.post(url=f'https://api.telegram.org/bot{TG_TOKEN}/sendMessage', 
                  data={'chat_id': TG_CHAT_ID, 'text': message, 'parse_mode': 'HTML'})
...
@dag(
    ...
    schedule='@daily',
    catchup=False,
    description='Автоматизация обработки данных для Ашан',
    tags=['Spark', 'S3', 'aushan'],
...
def pipeline():
    ...
    wait_for_files = FileSensor(
        ...
        filepath='/opt/airflow/data/aushan*',
        mode='reschedule',
        poke_interval=30,
        timeout=60,
    ...
    def load_to_s3_raw():
        ...
        if files_processed == 0:
            logging.warning('⚠ Нет файлов Ашан')
        else:
    ...
    spark_processing = SparkSubmitOperator(
        ...
        application='jobs/aushan_spark_processing.py',
        on_failure_callback=telegram_alert,
        retries=1,
