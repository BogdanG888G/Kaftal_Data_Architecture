...
def telegram_success():
    requests.post(url=f'https://api.telegram.org/bot{TG_TOKEN}/sendMessage', data={'chat_id': TG_CHAT_ID, 'text': '✅ Паплайн по Пятерочка завершился', 'parse_mode': 'HTML'})

default_args = {'retries': 2, 'retry_delay': datetime.timedelta(minutes=1)}

@dag(dag_id='etl_pyaterochka', start_date=datetime.datetime(year=2026, month=2, day=1), schedule='@daily', catchup=False,
     description='Автоматизация обработки данных для Пятерочка', tags=['Spark', 'S3', 'pyaterochka'],
...
def pipeline():
    ...
    wait_for_files = FileSensor(task_id='wait_for_files', fs_conn_id='folder_connect',
        filepath='/opt/airflow/data/pyaterochka*', mode='reschedule', poke_interval=30,
    ...
    spark_processing = SparkSubmitOperator(task_id='spark_processing', conn_id='spark_connection',
        trigger_rule=TriggerRule.ALL_DONE, application='jobs/pyaterochka_spark_processing.py',
