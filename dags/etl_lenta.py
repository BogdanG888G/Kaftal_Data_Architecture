...
def telegram_success(context):
    ...
    message = (
        f"✅ <b>Пайплайн по Ленте успешно завершился</b>\n"
...
@dag(
    ...
    schedule="@daily",
    catchup=False,
    description="Автоматизация обработки данных для Ленты",
    tags=["Spark", "S3", "lenta"],
...
def pipeline():
    ...
    wait_for_files = FileSensor(
        ...
        filepath="/opt/airflow/data/lenta_*.csv",
        mode="reschedule",
        poke_interval=30,
        timeout=60,
    ...
    def load_to_s3_raw():
        ...
        for file in os.listdir(input_folder):
            if not (file.startswith("lenta_") and file.endswith(".csv")):
                continue
            ...
            logging.info(f"✓ Загрузили {file} в raw")
        ...
        if files_processed == 0:
            logging.warning("⚠ Нет файлов Лента")
        else:
    ...
    spark_processing = SparkSubmitOperator(
        ...
        application="jobs/lenta_spark_processing.py",
        on_failure_callback=telegram_alert,
        retries=1,
