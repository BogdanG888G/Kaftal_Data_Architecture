...
def telegram_success(context):
    requests.post(url=f'https://api.telegram.org/bot{TG_TOKEN}/sendMessage',
                  data={'chat_id': TG_CHAT, 'text': 'Пайплайн по X5 (Кондитерка) успешно завершился ✅', 'parse_mode': 'HTML'})

@dag(
    dag_id='x5_konditerka_etl',
    description='Автоматизация обработки данных для X5 (Кондитерка)',
    catchup=False,
    start_date=datetime.datetime(year=2026, month=4, day=1),
    schedule='@daily',
    tags=['X5', 'S3', 'Spark', 'Konditerka'],
...
def pipeline():
        ...
        for file in folder:
            if 'x5' in file:
                s3.load_file(
                    ...
                    key=file,
                    bucket_name='rawkonditerka' # ИЗМЕНЕНО
                )
    ...
    spark_processing = SparkSubmitOperator(
        ...
        application='jobs/x5_konditerka_spark.py', # ИЗМЕНЕНО
        on_failure_callback=telegram_alert,
        deploy_mode='client',
        conf={
            'spark.jars.packages': (
            ...
            'spark.sql.catalog.iceberg.warehouse': 's3a://warehousekonditerka/', # ИЗМЕНЕНО: Направляем Iceberg в новый бакет
