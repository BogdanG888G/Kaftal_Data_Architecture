...
def telegram_success(context):
    requests.post(url=f'https://api.telegram.org/bot{TG_TOKEN}/sendMessage',
                  data={'chat_id': TG_CHAT, 'text': 'Пайплайн по Магниту (Кондитерка) успешно завершился ✅', 'parse_mode': 'HTML'})

@dag(
    dag_id='magnit_konditerka_etl',
    description='Автоматизация обработки данных для Магнита (Кондитерка)',
    catchup=False,
    start_date=datetime.datetime(year=2026, month=4, day=1),
    schedule='@daily',
    tags=['Magnit', 'S3', 'Spark', 'Konditerka'],
...
def pipeline():
    ...
    wait_for_data = FileSensor(
        ...
        filepath='/opt/airflow/data/magnit*',
        mode='reschedule',
        timeout=60,
    ...
    def load_to_s3():
        ...
        for file in folder:
            if 'magnit' not in file:
                continue
            try:
                if s3.check_for_key(key=file, bucket_name='rawkonditerka'):
                    logging.warning(f'⊘ Файл {file} уже есть в rawkonditerka — пропускаем загрузку')
                else:
                    s3.load_file(
                    ...
                    logging.info(f'✓ Успешно загружен файл {file} в rawkonditerka')
    ...
    spark_processing = SparkSubmitOperator(
        ...
        application='jobs/magnit_konditerka_spark.py',
        on_failure_callback=telegram_alert,
        deploy_mode='client',
        conf={
            'spark.jars.packages': (
