...
def telegram_success(context):
    requests.post(url=f'https://api.telegram.org/bot{TG_TOKEN}/sendMessage',
                  data={'chat_id': TG_CHAT, 'text': 'Пайплайн по Ашану (Кондитерка) успешно завершился ✅', 'parse_mode': 'HTML'})

@dag(
    dag_id='auchan_konditerka_etl',
    description='Автоматизация обработки данных для Ашана (Кондитерка)',
    catchup=False,
    start_date=datetime.datetime(year=2026, month=4, day=1),
    schedule='@daily',
    tags=['Auchan', 'S3', 'Spark', 'Konditerka'],
...
def pipeline():
    ...
    wait_for_data = FileSensor(
        ...
        filepath='/opt/airflow/data/auchan*',
        mode='reschedule',
        timeout=60,
    ...
    spark_processing = SparkSubmitOperator(
        ...
        application='jobs/auchan_konditerka_spark.py',
        on_failure_callback=telegram_alert,
        deploy_mode='client',
        conf={
            'spark.jars.packages': (
