...
# ОСНОВНОЙ ЦИКЛ
# ============================================================
# ИЗМЕНЕНО: читаем из rawkonditerka
...
if 'Contents' not in objects:
    ...
    for obj in files:
        ...
        # ШАГ 3: Месяц и год из имени файла (ИЗМЕНЕНА ЛОГИКА ИНДЕКСОВ)
        # Пример: x5_halva_april_2026_part1.csv
        # parts[0]="x5", parts[1]="halva", parts[2]="april", parts[3]="2026"
        parts = file.split('_')
        month_str = MONTH_MAPPING.get(parts[2].lower())
        month_int = MONTH_MAPPING_INT.get(month_str)
        
        # Обрабатываем год, чтобы отрезать .csv если нет суффикса part1
        ...
        # ШАГ 10: Запись (ИЗМЕНЕНО ИМЯ ТАБЛИЦЫ ICEBERG)
        ...
        if not file_exists:
            ...
            # Убедись, что база konditerka_silver создана! 
            spark.sql("CREATE NAMESPACE IF NOT EXISTS iceberg.konditerka_silver")
            
            final_df.writeTo(TARGET_TABLE).partitionedBy('retail_chain', 'year', 'month').append()
