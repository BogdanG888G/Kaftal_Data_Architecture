...
try:
    ...
    if not dfs:
        ...
        # ----------------------------------------------------------
        # Мы оставляем только картофельные/кукурузные чипсы, отсекая морепродукты, семечки и прочее.
        # Фильтруем по категории, по вхождению слова "чипс" в название или по заполненному типу из справочника.
        all_data = all_data.filter(
           F.lower(F.coalesce(F.col('product_name'), F.lit(''))).contains('чипс')
        )
