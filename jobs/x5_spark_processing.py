...
SILVER_TYPES = {
    ...
    'product_id': 'int', 'product_name': 'string', 'brand': 'string', 'vendor': 'string',
...
# ОСНОВНОЙ ЦИКЛ
# ============================================================
objects = s3.list_objects_v2(Bucket='raw', Prefix='x5_')

if 'Contents' not in objects:
    print("⚠ Нет файлов x5_ в бакете raw")
else:
    ...
    for obj in files:
        ...
        # X5 имеет хитрый формат — 2 строки заголовка
        df = spark.read.csv(file_name, sep=';')
        df = df.offset(1)  # пропускаем первую строку
        
        first_row = list(df.first())[:-1]
        df = df.offset(1)  # пропускаем ещё одну
        ...
        # ШАГ 4: Нормализация названий сетей
        ...
        # ШАГ 5: Запятые → точки
        for col_name in ['sales_amount_rub', 'sales_cost_price', 'average_cost_price', 'average_sell_price']:
        ...
        print(f'NULL колонки: {remaining_cols}')
        ...
        # ШАГ 8: Финальный датафрейм + фильтрация
        final_df = df.select(*SILVER_COLUMNS)
        
        final_df = final_df.filter(
            df['sales_quantity'].isNotNull() & (df['sales_quantity'] > 0)
        ).filter(
            df['sales_amount_rub'].isNotNull() & (df['sales_amount_rub'] > 0)
        ).filter(
            df['sales_cost_price'].isNotNull() & (df['sales_cost_price'] > 0)
        )
        ...
        # ШАГ 10: Запись
        try:
            res = spark.sql(f"SELECT COUNT(*) FROM iceberg.x5_silver.sales WHERE file_name = '{file[:-4]}'").first()
            file_exists = res[0] > 0
        except:
            file_exists = False
