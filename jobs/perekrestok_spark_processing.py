...
SILVER_COLUMNS = [
    ...
    'store_code', 'store_name', 'rc',
    'product_category_3', 'product_category_4', 'base_type',
    'product_id', 'product_name', 'product_uni_name',
...
MAPPING_V2 = {
    ...
    'Тип основы': 'base_type', 'Поставщик': 'vendor', 'Бренд': 'brand',
    'УНИ Наименование': 'product_uni_name', 'Наименование': 'product_name',
    'Граммовка': 'weight_grams', 'Вкус': 'flavor',
    'Продажи, шт': 'sales_quantity', 'Продажи, руб': 'sales_amount_rub',
    'Продажи, тонн': 'sales_tons', 'Себест. Руб': 'sales_cost_price',
    'ТТ': 'store_name_full',
}
...
s3 = boto3.client('s3', endpoint_url='http://minio:9000', aws_access_key_id='minioadmin', aws_secret_access_key='minioadmin')
objects = s3.list_objects_v2(Bucket='raw', Prefix='perekrestok_')

if 'Contents' not in objects:
    print("⚠ Нет файлов perekrestok_")
else:
    ...
    for obj in objects['Contents']:
        ...
        # ШАГ 2: Служебные
        df = df.withColumn('retail_chain', F.lit('Перекресток'))
        ...
        try:
            res = spark.sql(f"SELECT COUNT(*) FROM iceberg.perekrestok_silver.sales WHERE file_name = '{file[:-4]}'").first()
            file_exists = res[0] > 0
        ...
        if not file_exists:
            ...
            final_df.writeTo('iceberg.perekrestok_silver.sales').partitionedBy('retail_chain', 'year', 'month').append()
...
print('✅ Обработка Перекресток завершена!')